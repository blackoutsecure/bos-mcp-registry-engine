...
function getGeneratedPageProfile() {
  const generatedPage = PROJECT_CONFIG.metadata.generatedPage || {};

  return {
    generatedBy:
      generatedPage.generatedBy || PROJECT_CONFIG.metadata.name || 'MCP Registry Engine',
    maintainedBy:
      generatedPage.maintainedBy ||
      generatedPage.sponsoredBy ||
      PROJECT_CONFIG.metadata.copyrightHolder ||
      'Maintainer',
    maintainerUrl:
      generatedPage.maintainerUrl ||
      generatedPage.sponsorUrl ||
      'https://blackoutsecure.app',
    footerGeneratedByPrefix:
      generatedPage.footerGeneratedByPrefix || 'Generated by',
    footerMaintainedByPrefix:
      generatedPage.footerMaintainedByPrefix ||
      generatedPage.footerSponsoredByPrefix ||
      'Maintained by',
    commentHeader:
      generatedPage.commentHeader || 'Generated file metadata',
    commentGeneratedByLabel:
      generatedPage.commentGeneratedByLabel || 'Generated by',
    commentMaintainerLabel:
      generatedPage.commentMaintainerLabel ||
      generatedPage.commentSponsoredByLabel ||
      'Maintained by',
    commentProjectUrlLabel:
      generatedPage.commentProjectUrlLabel ||
      generatedPage.commentSponsorUrlLabel ||
      'Project URL',
    commentFormatVersionLabel:
      generatedPage.commentFormatVersionLabel || 'Metadata standard',
    commentFormatVersion: generatedPage.commentFormatVersion || 'oss-v1',
  };
}

function buildGeneratedPageComment(profile) {
  return `  <!--
    ${profile.commentHeader}
    ${profile.commentGeneratedByLabel}: ${profile.generatedBy}
    ${profile.commentMaintainerLabel}: ${profile.maintainedBy}
    ${profile.commentProjectUrlLabel}: ${profile.maintainerUrl}
    ${profile.commentFormatVersionLabel}: ${profile.commentFormatVersion}
  -->`;
}

function buildGeneratedPageFooter(profile) {
  return `    <footer>
      <p>${profile.footerGeneratedByPrefix} ${profile.generatedBy}.</p>
      <p>
        ${profile.footerMaintainedByPrefix}
        <a href="${profile.maintainerUrl}">${profile.maintainedBy}</a>.
      </p>
    </footer>`;
}

function buildRootIndexHtml(registryVersion) {
  const generatedPageProfile = getGeneratedPageProfile();
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body>
${buildGeneratedPageComment(generatedPageProfile)}
    <main>
...
${buildGeneratedPageFooter(generatedPageProfile)}
  </body>
</html>
`;
}
